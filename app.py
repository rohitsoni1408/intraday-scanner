...
def load_nse_750_symbols():
    ...
    core_symbols = [
        ...
        "PERSISTENT", "POLYCAB", "DIXON", "COFORGE", "MPHASIS", "ASTRAL", "SUPREMEIND", "PAGEIND", "MUTHOOTFIN",
        "CHOLAFIN", "ASHOKLEY", "OBEROIRLTY", "BALKRISIND", "CUMMINSIND", "TIINDIA", "MAXHEALTH", "LUPIN",
        "AUROPHARMA", "BOSCHLTD", "BHARATFORG", "PIIND", "SRF", "IDEA", "YESBANK", "IDFCFIRSTB", "TATAINVEST",
        "KPRMILL", "NAUKRI", "KALYANKJIL", "OFSS", "RADICO", "FEDERALBNK", "AJANTPHARM", "TATACOMM", "EXIDEIND",
        "OIL", "PETRONET", "LICHSGFIN", "ESCORTS", "GODREJPROP", "ABBOTINDIA", "BHEL", "IRFC", "RVNL", "MAZDOCK"
    ]
    symbols.update(core_symbols)
    return list(symbols)

NSE_750_POOL = load_nse_750_symbols()
...
def fetch_live_market_data(symbols):
    ...
    for sym in symbols:
        ...
        try:
            ...
            intra_bear_div = (float(df_intraday['High'].iloc[-1]) >= float(df_intraday['High'].iloc[-6])) and (curr_rsi < prev_rsi)

            data_dict[clean_sym] = {
                "cmp": cmp, "day_open": day_open, "prev_close": prev_close,
                "chg": pct_change, "vol": volume, "day_high": day_high,
                "day_low": day_low, "vwap": vwap,
                "rsi_bull_div": intra_bull_div, "rsi_bear_div": intra_bear_div
            }
        except Exception:
            continue
    return data_dict
...
def process_ultimate_confluence(stock_data, top_n_count, force_post_market=False):
    ...
    active_symbols = list(dict.fromkeys(extracted_symbols + NSE_750_POOL[:40]))
    ...
    for symbol in active_symbols:
        ...
        rsi_bull_div, rsi_bear_div = live_info['rsi_bull_div'], live_info['rsi_bear_div']
        ...
        if 1.0 <= abs(pct_change) <= 3.5: green_reasons.append("Healthy Momentum")
...
def fetch_weekly_mtf_strategy(symbols):
    ...
    for sym in symbols:
        ...
        try:
            ...
            elif (bearish_rsi_div or macd_bearish_cross) and (cmp >= best_supply_zone * 0.93):
                is_buy = False
...
def run_live_backtest(target_date, scan_clause, top_n_count):
    ...
    stock_list = list(dict.fromkeys(extracted_symbols + NSE_750_POOL[:20]))[:top_n_count*2]
    results = []
    ...
    for symbol in stock_list:
        try:
            ...
            if is_buy:
                ...
                if max_price >= t2: status, pnl_val = "🎯 Target 2 Hit", round(((t2 - entry_price) / entry_price) * 100, 2)
                elif max_price >= t1: status, pnl_val = "🎯 Target 1 Hit", round(((t1 - entry_price) / entry_price) * 100, 2)
                elif min_price <= sl: status, pnl_val = "🛑 SL Hit", round(((sl - entry_price) / entry_price) * 100, 2)
                ...
                if min_price <= t2: status, pnl_val = "🎯 Target 2 Hit", round(((entry_price - t2) / entry_price) * 100, 2)
                elif min_price <= t1: status, pnl_val = "🎯 Target 1 Hit", round(((entry_price - t1) / entry_price) * 100, 2)
                elif max_price >= sl: status, pnl_val = "🛑 SL Hit", round(((entry_price - sl) / entry_price) * 100, 2)
...
with col_info:
    ...
    st.info(f"Market Status: **{market_status}** | Universe: **Top NSE 750 Stocks**")
with col_slider:
    selected_count = st.slider("Select Stock Output Count:", min_value=3, max_value=25, value=10, step=1)
...
with main_tab1:
    ...
    if st.button("🚀 Run Intraday Engine Scan", type="primary", use_container_width=True):
        with st.spinner("Scanning top NSE 750 stocks for intraday setups..."):
...
with main_tab2:
    ...
    if st.button("🚀 Run Intraday Backtest", type="primary"):
        with st.spinner("Executing backtest over NSE 750 intraday candles..."):
...
# --- TAB 3: WEEKLY MTF ENGINE ---
with main_tab3:
    st.subheader("🗓️ Weekly MTF Strategy (Volume Profile POC + VWAP + RSI Div)")
    st.markdown("Multi-timeframe scanner combining **Volume Profile Point of Control (POC)**, **Weekly VWAP**, **Demand/Supply Zones**, and **RSI Divergence** across top NSE 750 stocks.")

    if st.button("🚀 Run Weekly MTF Scan", type="primary", use_container_width=True):
        with st.spinner("Analyzing volume profiles, weekly VWAP, and RSI divergences across NSE 750 stocks..."):
            df_mtf = fetch_weekly_mtf_strategy(NSE_750_POOL)
